...
/// <summary>
/// Витрина твика: в какой блок попадает, что именно меняет и о чём предупредить.
/// Держим отдельно от самих твиков, чтобы вся раскладка была видна в одном месте
/// и её можно было менять, не трогая логику применения.
/// </summary>
public sealed record TweakInfo(string GroupId, string Target, string? Tag = null, string? Warning = null);

/// <summary>
/// Раскладка твиков по блокам. Ключи реестра и команды взяты ИЗ КОДА твиков,
/// а не из макета — в интерфейсе должно быть написано то, что программа
